...
func main() {
	...
	var k Cipher = &kuznechik.Kuznechik{Key: key}
	k.SetKey()
	...
	f, err := os.Create("plaintext-original")
	if err != nil {
		panic(err)
	}

	for i := 0; i < 781250; i++ {
		_, _ = f.Write(plaintext[:])
	}
	...
	go processTestEncrypt(i1, c1, k)
	go processTestEncrypt(i2, c1, k)
	go processTestEncrypt(i3, c1, k)
	go processTestEncrypt(i4, c1, k)
	go processTestEncrypt(i5, c1, k)
	go processTestEncrypt(i6, c1, k)
	go processTestEncrypt(i7, c1, k)
	go processTestEncrypt(i8, c1, k)
	go processTestEncrypt(i9, c1, k)
	go processTestEncrypt(i10, c1, k)
...
func processTestEncrypt(data [][]byte, c chan [][]byte, cipher Cipher) {
	r := rand.Intn(100)
	t := time.Now()
